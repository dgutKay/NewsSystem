...
public class JacobWordManager {
	...
	// constructor
	public JacobWordManager(boolean visible) {
	 ...
	 * 
	 */
	public void closeDocumentWithSave() {
		if (doc != null) {
	...
	public void clearFont2() {
		this.setFont(false, false, false, "0,0,0", "15.5", "仿宋_GB2312");
	}
	...
	public void delLastRowFromTable(int tableIndex) {
		Dispatch tables = Dispatch.get(doc, "Tables").toDispatch();
		Dispatch table = Dispatch.call(tables, "Item", new Variant(tableIndex)).toDispatch();
		Dispatch rows = Dispatch.get(table, "Rows").toDispatch();
	 ...
	 *            另一个文档的磁盘路径
	 */
	public void copyContentFromAnotherDocInsertAfter(String anotherDocPath) {
	...
	public void copyContentFromAnotherDocInsertBefore(String anotherDocPath) {
		Dispatch wordContent = Dispatch.get(doc, "Content").toDispatch(); // 取得当前文档的内容
		Dispatch.call(wordContent, "InsertBefore", "$selection$");// 插入特殊符定位插入点
		copyContentFromAnotherDoc(anotherDocPath, "$selection$");
	}
	...
	public void callMacro(String macroName) {
		Dispatch.call(word, "Run", new Variant(macroName));// 在这个文档上运行宏
