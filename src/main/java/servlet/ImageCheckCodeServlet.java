...
public class ImageCheckCodeServlet extends HttpServlet {
	...
	private void setBackground(Graphics gra) {
		...
		gra.fillRect(0, 0, WIDTH, HEIGHT);// 把图片颜色给填充
	}

	private void setBorder(Graphics gra) {
		gra.setColor(Color.BLUE);
		gra.drawRect(0, 0, WIDTH - 2, HEIGHT - 2);
	}

	private void drawRandomLine(Graphics gra) {
	...
	private String writeRandomNum(Graphics gra) {
		// [\u4e00-\u9fa5]是汉字的区间正则表达式,\\u是代表是存在uncode码表中,意思就是汉字都在\U（uncode）码表中4e00到9fa5之间
