...
<script type="text/javascript">
	function checkAUser(userId) {
		$("#ids").val(userId);
		$("#rightDiv").load("/NewsSystem/servlet/UserServlet?condition=check", $("#myform").serialize());
	}
	...
	<form action="" id="myform" method="post">
		<table border="1" align="center" cellpadding="5" cellspacing="0">
			<tr bgcolor='lightyellow'>
				...
				<td>Register Date</td>
				<td><a href='javascript:void(0);'
					onclick="getOnePage('','usability');">Usability</a></td>
				<td>Change Usability</td>
			</tr>
			<c:forEach items="${requestScope.users }" var="user">
				<tr>
					...
					<td><c:if test="${user.usability=='use'}">
							<a href="javascript:void(0);"
								onclick="checkAUser('${user.userId }');">Stop</a>
						</c:if> <c:if test="${user.usability=='stop'}">
							<a href="javascript:void(0);"
								onclick="checkAUser('${user.userId }');">Use</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</table>
